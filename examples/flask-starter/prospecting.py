...
def ai_filter_candidates_at_company(
    ...
    prompt = (
        ...
        f"- Investor / advisor / board-only role (e.g. 'Investor at {{Co}}', "
        f"  'Board Member', 'Angel Investor') → confidence ≤ 0.30 UNLESS the "
        f"  TARGET COMPANY is itself a VC fund, private-equity firm, family "
        f"  office, asset manager, or similar investment vehicle (judge from "
        f"  the company name + LinkedIn URL). For an operational company "
        f"  (SaaS, fintech, consumer, etc.), an investor title is NOT a "
        f"  current-employee match — they're a stakeholder, not staff.\n\n"
        f"CANDIDATES:\n{candidates_block}\n\n"
